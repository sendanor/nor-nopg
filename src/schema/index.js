...
--
CREATE OR REPLACE FUNCTION plv8_init() RETURNS boolean LANGUAGE plv8 VOLATILE AS $$
plv8._modules = {};
...
$js$;
		...
		ALTER SEQUENCE objects_seq OWNED BY objects.id;
		CREATE INDEX objects_type ON objects (types_id);
		CREATE TRIGGER types_modified BEFORE UPDATE ON types FOR EACH ROW EXECUTE PROCEDURE moddatetime (modified);
